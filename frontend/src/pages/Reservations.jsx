...
const STATUS = { PENDING: { label: '待取货', color: '#f7c948', bg: '#fffbeb' }, COMPLETED: { label: '已完成', color: '#16a34a', bg: '#f0fff4' }, CANCELLED: { label: '已取消', color: '#dc2626', bg: '#fff0f0' } }
...
export default function Reservations() {
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      {msg && <div style={{ padding: '12px 16px', borderRadius: 8, marginBottom: 16, fontSize: 14, background: msg.startsWith('✅') ? '#f0fff4' : '#fff0f0', color: msg.startsWith('✅') ? '#16a34a' : '#dc2626' }}>{msg}</div>}
      {list.length === 0 ? (
            ...
            {r.status === 'PENDING' && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #f0f0f0', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {!isBuyer && completing !== r.id && (
                  <button onClick={() => setCompleting(r.id)} style={{ background: '#ff6b35', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>核销取货码</button>
                )}
                {completing === r.id && (
                  <>
                    <input value={codeInput} onChange={e => setCodeInput(e.target.value)} placeholder="输入取货码"
                      style={{ padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: 8, fontSize: 13, outline: 'none', width: 140 }} />
                    <button onClick={() => handleComplete(r.id)} style={{ background: '#16a34a', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13 }}>确认完成</button>
                    <button onClick={() => setCompleting(null)} style={{ background: '#f0f0f0', color: '#555', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13 }}>取消</button>
                  </>
                )}
                {isBuyer && (
                  <button onClick={() => handleCancel(r.id)} style={{ background: '#f0f0f0', color: '#555', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13 }}>取消预约</button>
                )}
              </div>
            )}
          </div>
